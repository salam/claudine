...
const VsCodeAdapter_1 = require("./platform/VsCodeAdapter");
...
const BoardExporter_1 = require("./services/BoardExporter");
const constants_1 = require("./constants");
let kanbanProvider;
let claudeCodeWatcher;
let stateManager;
let storageService;
let imageGenerator;
...
async function activate(context) {
    ...
    const platform = new VsCodeAdapter_1.VsCodeAdapter(context);
    // Initialize services
    storageService = new StorageService_1.StorageService(platform);
    stateManager = new StateManager_1.StateManager(storageService, platform);
    imageGenerator = new ImageGenerator_1.ImageGenerator(storageService, platform);
    claudeCodeWatcher = new ClaudeCodeWatcher_1.ClaudeCodeWatcher(stateManager, platform, imageGenerator);
    ...
    commandProcessor = new CommandProcessor_1.CommandProcessor(stateManager, platform);
    commandProcessor.startWatching();
    // Initialize the Kanban view provider
    kanbanProvider = new KanbanViewProvider_1.KanbanViewProvider(context.extensionUri, stateManager, claudeCodeWatcher);
    ...
    const legacyKey = vscode.workspace.getConfiguration('claudine').get('imageGenerationApiKey', '');
    if (legacyKey) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('claudine.kanbanView', kanbanProvider, webviewOptions), vscode.window.registerWebviewViewProvider('claudine.kanbanViewSidebar', kanbanProvider, webviewOptions));
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand('claudine.openKanban', () => {
        const location = vscode.workspace.getConfiguration('claudine').get('viewLocation', 'panel');
        const viewId = location === 'sidebar' ? 'claudine.kanbanViewSidebar' : 'claudine.kanbanView';
        vscode.commands.executeCommand(`${viewId}.focus`);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('claudine.refresh', () => {
        claudeCodeWatcher.refresh();
        kanbanProvider.refresh();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('claudine.closeEmptyClaudeTabs', () => {
        kanbanProvider.closeEmptyClaudeTabs();
    }));
    // Open Conversation — QuickPick list of all conversations
    context.subscriptions.push(vscode.commands.registerCommand('claudine.openConversation', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.searchConversations', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.newConversation', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.moveConversation', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.showNeedsInput', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.showInProgress', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.archiveDone', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.toggleSummarization', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.toggleSearch', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.regenerateIcons', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.openSettings', () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.togglePlacement', async () => {
        const cfg = vscode.workspace.getConfiguration('claudine');
        const current = cfg.get('viewLocation', 'panel');
        ...
        setTimeout(() => {
            ...
            vscode.commands.executeCommand(`${viewId}.focus`);
        }, constants_1.VIEW_SWITCH_DELAY_MS);
        vscode.window.showInformationMessage(vscode.l10n.t('Claudine moved to {0}.', next === 'sidebar' ? vscode.l10n.t('sidebar') : vscode.l10n.t('panel')));
    }));
    // Export Board — save conversations as CSV, JSON, or Trello format
    context.subscriptions.push(vscode.commands.registerCommand('claudine.exportBoard', async () => {
        const conversations = stateManager.getConversations();
        if (conversations.length === 0) {
            vscode.window.showInformationMessage(vscode.l10n.t('No conversations to export.'));
            return;
        }
        await (0, BoardExporter_1.promptExport)(conversations);
    }));
    // Import Board — load conversations from a Claudine JSON export
    context.subscriptions.push(vscode.commands.registerCommand('claudine.importBoard', async () => {
        const imported = await (0, BoardExporter_1.promptImport)();
        if (imported) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.setupAgentIntegration', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.showDiagnostics', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('claudine.focusClaude', async () => {
    ...
    const hasSeenWelcome = context.globalState.get('claudine.hasSeenWelcome', false);
    if (!hasSeenWelcome) {
        ...
        vscode.window.showInformationMessage(vscode.l10n.t('Claudine is ready! A kanban board for your Claude Code conversations.'), walkthroughAction, openAction).then(selection => {
            if (selection === walkthroughAction) {
                vscode.commands.executeCommand('workbench.action.openWalkthrough', 'claudine.claudine#claudine.gettingStarted', true);
            }
    ...
    if (workspaceRoot && stateManager.getConversations().length > 0) {
        ...
        if (!fs.existsSync(agentsFile)) {
            ...
            vscode.window.showInformationMessage(vscode.l10n.t('Enable Claude Code agents to control the Claudine board? This creates a CLAUDINE.AGENTS.md file you can reference from CLAUDE.md.'), scaffold, later).then(selection => {
    ...
    context.subscriptions.push(stateManager.onNeedsInput(conv => {
        const openAction = vscode.l10n.t('Open');
        vscode.window.showInformationMessage(vscode.l10n.t('"{0}" needs your input', conv.title), openAction).then(selection => {
    ...
    context.subscriptions.push(stateManager.onRateLimitDetected(conv => {
        const resetDisplay = conv.rateLimitResetDisplay || 'soon';
        vscode.window.showWarningMessage(vscode.l10n.t('Rate limit hit — resets {0}', resetDisplay));
        // Schedule auto-restart if the setting is enabled
        const autoRestart = vscode.workspace.getConfiguration('claudine').get('autoRestartAfterRateLimit', false);
    ...
    return {
        ...
        getConversation: (id) => stateManager.getConversation(id),
        getConversationsByStatus: (status) => stateManager.getConversationsByStatus(status),
