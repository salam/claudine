...
describe('BUG2b — Zoom compensation math', () => {
  /**
   ...
   */
  function logicalWidth(rectWidth: number, zoom: number): number {
    return Math.round(rectWidth / zoom);
  }

  function adjustedDelta(deltaX: number, zoom: number): number {
    return deltaX / zoom;
  }

  /** Inverse percentage used for width/height so the scaled result fills the wrapper. */
  function inversePercent(zoom: number): number {
    return 100 / zoom;
  }

  it('at zoom 1.0 no compensation is needed', () => {
    expect(logicalWidth(300, 1.0)).toBe(300);
    expect(adjustedDelta(10, 1.0)).toBe(10);
    expect(inversePercent(1.0)).toBe(100);
  });

  it('at zoom 1.5 scales down correctly', () => {
    // A 300px-wide element at 1.5x reports 450px via getBoundingClientRect
    expect(logicalWidth(450, 1.5)).toBe(300);
    // A 15px mouse drag at 1.5x corresponds to 10 logical pixels
    expect(adjustedDelta(15, 1.5)).toBe(10);
    // Board width should be 100/1.5 ≈ 66.67% so after scaling it fills the wrapper
    expect(inversePercent(1.5)).toBeCloseTo(66.667, 2);
  });

  it('at zoom 0.5 scales up correctly', () => {
    expect(logicalWidth(150, 0.5)).toBe(300);
    expect(adjustedDelta(5, 0.5)).toBe(10);
    // Board width should be 200% so after 0.5x scale it fills the wrapper
    expect(inversePercent(0.5)).toBe(200);
  });

  it('at zoom 0.7 compensates fractional zoom', () => {
    // 300 * 0.7 = 210 from getBoundingClientRect
    expect(logicalWidth(210, 0.7)).toBe(300);
    expect(adjustedDelta(7, 0.7)).toBe(10);
    expect(inversePercent(0.7)).toBeCloseTo(142.857, 2);
  });

  it('inverse percent * zoom always equals 100', () => {
    for (const z of [0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4, 1.5]) {
      expect(inversePercent(z) * z).toBeCloseTo(100, 10);
    }
  });
});
